...
data_path = 'data/'
hdf_database_names = ['5699_0_reconstruction_goblet_ID_0_of_4.hdf5',
                      '5699_1_reconstruction_goblet_ID_1_of_4.hdf5',
                      '5699_2_reconstruction_goblet_ID_2_of_4.hdf5',
                      '5699_3_reconstruction_goblet_ID_3_of_4.hdf5']
...
def find_and_plot(ur, image, algorithm, pattern):
    image_found = False
    for idx, hdf_database_name in enumerate(hdf_database_names):
        ...
        with tb.File(data_path + hdf_database_name, mode='r') as h5_file:
            ...
            try:
                if (ur == 0.225):
                    df_current = reconstruction_metrics.query('delta > @ur - 0.001 and delta <= @ur and image == @image and reconstruction_algorithm == @algorithm and sampling_pattern == @pattern')
                else:
                ...
                plt.savefig('../figures_test/recon-{}-{}-{}-{}-256px.pdf'.format(image.replace('.mi',''),
                                                                      algorithm,
                                                                      pattern,
                                                                      str(ur).replace('.','_')),
                            bbox_inches='tight',
                            pad_inches=0,
                            dpi=82.75) # dpi was chosen to match figzise in line 60 to obtain 256x256 resolution
                plt.close()
                break
            finally:
                h5_file.close()
    ...
    for image in images:
        for algorithm in reconstruction_algorithms:
            for pattern in sampling_patterns:
                find_and_plot(ur, image, algorithm, pattern)
